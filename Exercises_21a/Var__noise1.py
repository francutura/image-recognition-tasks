...
def call__make_predictions__with__filtering(i_orig, if_show=False, if_open=False):
    ...
    if if_open:
        in1_filtered1 = cv2.morphologyEx(in1_filtered1.reshape(28, 28).astype('uint8'), cv2.MORPH_OPEN,
                                         np.ones((2, 2), np.uint8))
    else:
        in1_filtered1 = cv2.morphologyEx(in1_filtered1.reshape(28, 28).astype('uint8'), cv2.MORPH_OPEN,
                                         np.ones((2, 2), np.uint8))
        in1_filtered1 = cv2.morphologyEx(in1_filtered1.reshape(28, 28).astype('uint8'), cv2.MORPH_CLOSE,
                                         np.ones((2, 2), np.uint8))
